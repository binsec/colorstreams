...
let opts = ref "--arjun 0 --delta 0.05 --epsilon 0.05"
...
let mk_cmd ~opts ~projections file =
    ...
    let cnf = File.create ~keep:true ((File.get_name file) ^ "_approxmc") ".cnf" in
    File.write_lines (projline::cnfs) cnf;
    if !PMCount.Solver.timeout > 0
    then Printf.sprintf "timeout -v %f approxmc %s %s" ((Float.of_int !PMCount.Solver.timeout) /. 1000.) opts @@ File.get_path cnf
    else Printf.sprintf "approxmc %s %s" opts @@ File.get_path cnf

let parse_out (out, err, (r : PMCount.Count.result)) =
    let resexp = Str.regexp {|s mc \([0-9]+\)|} in
        ...
        (
            ...
            (
                try
                    ...
                    Message.Wrap.send (Message.Wrap.BigWarning(lazy ("approxmc: timeout")));
...
let _ = 
    PMCount.Solver.register "approxmc" "Approximate model counter." (mk_cmd ~opts:!opts) parse_out;
    Options.register_option ("-approxmc-opts", Arg.Set_string(opts), Printf.sprintf "Set command line options for approxmc (default: %s)." !opts)
